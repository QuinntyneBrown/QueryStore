...
import { Action, StateChangedAction } from "./actions";
...
class extends base {
  ...
  private readonly _cacheKeyObservableMap: Map<CacheKey, Observable<any> | null> = new Map();
  ...
  private _toStateChangedAction(action: Action | Action[]): Action | null {
  ...
  private _isRefreshAction(action:Action[] | Action):boolean {
    if(action && action != this._toStateChangedAction(action) && !Array.isArray(action) && action.indexOf(this._id) > -1) {
      return true;
    }
    return false;
  }
  ...
  private _isNullifyAction(action: Action[] | Action): boolean {
    if(action && action != this._toStateChangedAction(action) && !this._isRefreshAction(action)) {
      return true;
    }
    return false;
  }
  ...
  protected from$<T>(observableFactory: {(): Observable<T>}, actionOrActions: Action | Action[] = []): Observable<T> {
    ...
    return dispatcher.pipe(
      filter(x => {
        if(this._anyActionsMappableTo(actions, x as string) && this._isRefreshAction(x)) {
          return true;
        }
        return false;
      }),
...
export const Store = store(class { })
